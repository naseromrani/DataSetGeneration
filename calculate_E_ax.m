function E_ax = calculate_E_ax(freq,  x_prim,  y_prim,  X_op,  M_x,  M_y,  theta_0,  phi_0,  epsilon_r,  h,  tc,  zeta_0, modulation_coeffs)
...
%         alpha_sw  =   calculate_alpha(freq, epsilon_r, h, X_op, m_rho, m_phi, theta_0, phi_0, tc, rho_prim, phi_prim)
        

%         E_rho0  =  (M_x) ./ sqrt(2*pi*sqrt(beta_sw^2  +  alpha_sw^2) .* rho_prim) .* exp(-alpha_sw * rho_prim);

        E_rho0 = (M_x) ./ sqrt(2*pi*sqrt(beta_sw^2  +  alpha_sw.^2) .* rho_prim) .* exp(-alpha_sw .* rho_prim);
%         E_rho0 = M_x * cos(pi / (2 * 10 * lambda) * rho_prim) ./ sqrt(2*pi*sqrt(beta_sw^2  +  alpha_sw^2) .* rho_prim) .* exp(-alpha_sw * rho_prim);
        E_ax  =  E_rho0 .* exp(-1i*(k*x_prim*sin(theta_0)*cos(phi_0) +  k*y_prim*sin(theta_0)*sin(phi_0)      +   zeta_0  +   tc*phi_prim  ));
        
        E_ax = E_ax .* (rho_prim < 7 * lambda);
